...
replace_docker_build_tag(){
  sed "2s/.*/tag=$1/" docker.build.sh > docker.build.sh.tmp && mv docker.build.sh.tmp docker.build.sh
}
replace_docker_build_tag $user_version
git add docker.build.sh
git commit -m "Release version $user_version"
git tag "$tag_name"

replace_docker_build_tag "SNAPSHOT"
git add docker.build.sh
